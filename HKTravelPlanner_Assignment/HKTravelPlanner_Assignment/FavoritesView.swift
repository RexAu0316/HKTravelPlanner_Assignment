    ...
    var body: some View {
        List {
            Section(header: Text("Favorite Locations")) {
                ForEach(sampleData.locations.filter { $0.isFavorite }) { location in
                    LocationRow(location: location)
                }
            }
            
            Section(header: Text("All Locations")) {
                ForEach(sampleData.locations) { location in
                    LocationRow(location: location)
                }
            }
            ...
            Section(header: Text("Categories")) {
                ForEach(["Transport Hub", "Shopping", "Dining", "Entertainment"], id: \.self) { category in
                    HStack {
                        ...
                        Text("\(sampleData.locations.filter { $0.category == category }.count)")
